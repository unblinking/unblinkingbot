...
async function getSnapshotUrls () {
  ...
  try {
    let output = await spawns.childProcess(
      ...
      [`--verbose`, `--log-level=debug`, `./lib/casper_snapshotUrls.js`]
    )

    await asyncForEach(output, async line => {
      console.log(line)
    })
    ...
    console.log('created the browser object')
    const page = await browser.newPage()
    console.log('created page object')
    await page.goto(`http://192.168.0.9:8765`)
    console.log('went to 192.168.0.9:8765, now waiting five seconds')
    await page.waitFor(5000)
    console.log('waited five seconds')
    let userName = `admin`
    ...
    console.log('done with keyboard.type')
