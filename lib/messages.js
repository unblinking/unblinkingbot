  ...
  try {
    ...
    if (Object.keys(snapshots).length === 0 && snapshots.constructor === Object) {
      ...
      let res = await slack.web.chat.postMessage(
        message.channel,
        `There are no snapshots configured. Set up a snapshot using the unblinkingBot's web based management console (typically located somewhere like http://127.0.0.1:1138/ on the same machine as the bot).`,
