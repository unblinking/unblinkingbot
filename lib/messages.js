...
async function getPreview (db, message, slack) {
  try {
    ...
    let res = slack.web.files.upload(`preview_${new Date().getTime()}.png`, {
      file: preview,
      filename: `preview_${new Date().getTime()}.jpg`,
      title: `Preview of motionEye`,
      channels: message.channel,
      initial_comment: `🖼 Here's that preview of the motionEye system that you wanted.`
    })
    return res
  } catch (e) {
    console.log(e.stack)
