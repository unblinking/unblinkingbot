...
function listen (db, io, slack) {
  io.on(`connection`, socket => {
    ...
    async function handleSaveSlackNotifyReq (notify, notifyType) {
      try {
        if (notify !== undefined && notifyType !== undefined &&
          ...
          let id
          if (notifyType === `channel`) id = getChannelIdByName(notify)
          else if (notifyType === `group`) id = getGroupIdByName(notify)
          else if (notifyType === `user`) id = getDmIdByUserName(notify)
    ...
    /**
     * Get the motionEye file motion.conf loaded status.
     */
    async function handleMotionConfFileReq () {
      try {
        let status = await motions.motionConfFileFound()
        socket.emit(`motionConfFileRes`, status)
      } catch (err) {
        console.log(err.message)
      }
    }
    ...
    /**
     * Get the motionEye file motioneye.conf loaded status.
     */
    async function handleMotionEyeConfFileReq () {
      try {
        let status = await motions.motionEyeConfFileFound()
        socket.emit(`motionEyeConfFileRes`, status)
      } catch (err) {
        console.log(err.message)
      }
    }
    ...
    /**
     * Get the motionEye thread files (one per camera) count.
     */
    async function handleMotionThreadFileReq () {
      try {
        let count = await motions.motionThreadFileCount()
