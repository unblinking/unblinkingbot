  ...
  return new Promise(resolve => {
    bundle.io.on('connection', socket => {
      ...
      socket.on('slackNotifyReq', () => {
        let data = {}
        getValuesByKeyPrefix(bundle, 'slack::settings::notify')
          .then(notify => data.notify = notify)
          .then(() => {
            return getValuesByKeyPrefix(bundle, 'slack::settings::notifyType')
          })
          .then(notifyType => {
      ...
      socket.on('slackTokenReq', () =>
        getValuesByKeyPrefix(bundle, 'slack::settings::token')
      ...
      socket.on('dashRecentActivityReq', async () => {
        try {
          const notify = await getValuesByKeyPrefix(bundle, `slack::settings::notify`)
          const notifyId = await getValuesByKeyPrefix(bundle, 'slack::settings::notifyId')
          const notifyType = await getValuesByKeyPrefix(bundle, 'slack::settings::notifyType')
          let history
          console.log(`dashRecentActivityReq`)
          if (notifyType === 'channel') {
            history = bundle.web.channels.history(notifyId, {
              'count': 5
            })
          } else if (notifyType === 'group') {
            history = bundle.web.groups.history(notifyId, {
              'count': 5
            })
          } else if (notifyType === 'user') {
            history = bundle.web.im.history(notifyId, {
              'count': 5
            })
          }
          if (history !== undefined && history.messages !== undefined) {
            ...
            bundle.io.emit('slacktivity', 'No default channel or no activity.')
          }
        } catch (err) {
          console.log(err)
        }
